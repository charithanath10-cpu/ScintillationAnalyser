...
LAMBDA_NAME    = os.environ.get("PROCESSOR_LAMBDA", "novatel_processor")
...
def invoke_processor_async(s3_key: str, filename: str,
    ...
    Lambda will write results to S3 at results/{session_id}.json.
    """
    payload = {
        "s3_key":     s3_key,
        "filename":   filename,
    ...
    try:
        ...
        print(f"[LAMBDA_CLIENT] Invoked {LAMBDA_NAME} async, status={status}")
        return status == 202
